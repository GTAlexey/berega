...
const deadlineTone = (due?: string, status?: string) => { if (!due || status === 'done') return 'ok'; const today = new Date(`${todayIso()}T00:00:00`); const target = new Date(`${due}T00:00:00`); const days = Math.round((target.getTime() - today.getTime()) / 86400000); return days < 0 ? 'danger' : days <= 3 ? 'warn' : 'ok'; };
...
function Home({ openKit, displayKits = kits, stages = stripSeededPeople(loadStoredArray('berega.stages', kitStages)), period, elements = [] }: { openKit: (id: string, target?: any) => void; displayKits?: any[]; stages?: any[]; period?: { from: string; to: string }; elements?: any[] }) {
  ...
  return <main className={`screen home-screen ${displayKits.length <= 4 ? 'few-kits' : ''}`}>
    ...
    <section className="kit-home-grid">
      {displayKits.map(k => { const blocked = blockersForKit(stages, k.id); const late = overdueForKit(stages, k.id); const problems = problemCardsForKit(stages, k.id); const stats = kitStepStats(stages, k.id, period); const kitState = kitStatusId(stages, k.id); const kitDeadline = kitDue[k.id] ?? latestStepDeadline(stages.filter((stage: any) => stage.kitId === k.id).flatMap((stage: any) => stage.steps)); const overdueDelta = late.length - (k.previousOverdue ?? 0); const blockedDelta = blocked.length - (k.previousBlockers ?? 0); return <Panel key={k.id} className="kit-status-card clickable-card" onClick={() => openKit(k.id)}>
        <button className="card-open" onClick={() => openKit(k.id)} aria-label={`Открыть ${k.name}`}></button>
        <div className="card-top"><div className="card-title"><h2>{k.name}</h2><p>{k.kind}</p></div><div className="home-card-meta"><span className={`deadline-button ${kitDeadline ? deadlineTone(kitDeadline, kitState) : 'empty'}`}>{shortDate(kitDeadline)}</span><span className={`status-word ${stepTone(kitState)}`}>{stepLabel(kitState)}</span><button className={`kit-location-word ${k.location === 'Pixso' || k.location === 'Pixco' ? 'ok' : ''}`}>{k.location === 'Pixso' || k.location === 'Pixco' ? '✓ Pixso' : k.location}</button></div></div>
...
function ElementCard({ element, kitsList = kits }: { element: any; kitsList?: any[] }) {
...
function GraphInfoPanel({ element, hoverElement, elements, setElements, actionMode, setActionMode, kitsList = kits }: { element: any; hoverElement: any; elements: any[]; setElements: (updater: any) => void; actionMode: 'kits' | 'replaces' | 'replaceBy' | null; setActionMode: (mode: 'kits' | 'replaces' | 'replaceBy' | null) => void; kitsList?: any[] }) {
...
function Graph({ selected, setSelected, elements, artifacts, setElements, displayKits = kits }: { selected: string | null; setSelected: (id: string | null) => void; elements: any[]; artifacts: any[]; setElements: (updater: any) => void; displayKits?: any[] }) {
  ...
  return <div className="graph-wrap"><Panel className="element-list-panel"><h3>Киты и элементы</h3>{actionMode && <p className="merge-pick-hint">{actionMode === 'kits' ? 'Выберите кит или элемент' : 'Выберите элемент для замещения'}</p>}{displayKits.map(k => <div key={k.id} className="left-kit-group"><button className="left-kit-title" style={{color: k.color.border}} onMouseEnter={() => setHovered(null)} onClick={() => toggleSelectedKit(k.id)}>{k.name}</button>{elements.filter(e => (e.kits ?? []).includes(k.id)).map(e => <button key={`${k.id}-${e.id}`} className={`element-list-item ${selected === e.id ? 'active' : ''} ${hovered === e.id ? 'hovered' : ''}`} onMouseEnter={() => setHovered(e.id)} onMouseLeave={() => setHovered(null)} onClick={() => applyMergePick(e.id)}>{e.name}</button>)}</div>)}<div className="left-kit-group"><b className="left-kit-title muted">Без кита</b>{elements.filter(e => !(e.kits ?? []).length).map(e => <button key={e.id} className={`element-list-item ${selected === e.id ? 'active' : ''} ${hovered === e.id ? 'hovered' : ''}`} onMouseEnter={() => setHovered(e.id)} onMouseLeave={() => setHovered(null)} onClick={() => applyMergePick(e.id)}>{e.name}</button>)}</div></Panel><div className="graph-canvas-panel" onMouseLeave={() => setHovered(null)}><ReactECharts className="echarts-graph" style={{ width: '100%', height: '100%' }} option={option} notMerge={true} onChartReady={(chart: any) => chart.getZr().on('click', (event: any) => { if (!event.target && !actionMode) setSelected(null); })} onEvents={{ mouseover: (params: any) => { if (params.dataType !== 'node') return; const id = params.data?.id; if (elements.some(e => e.id === id) || displayKits.some(k => k.id === id)) setHovered(id); }, mouseout: (params: any) => { if (params.dataType === 'node') setHovered(null); }, globalout: () => setHovered(null), click: (params: any) => { if (params.dataType === 'node' && elements.some(e => e.id === params.data?.id)) applyMergePick(params.data.id); if (params.dataType === 'node' && displayKits.some(k => k.id === params.data?.id)) toggleSelectedKit(params.data.id); } }} /></div><GraphInfoPanel element={selectedElement} hoverElement={hoverElement} elements={elements} setElements={setElements} actionMode={actionMode} setActionMode={setActionMode} kitsList={displayKits}/></div>;
}

function DetailsScreen({ initialKit, target, artifactList, setArtifactList, elements, setElements, kitEdits, setKitEdits, stages, setStages, extraKits, setExtraKits }: { initialKit: string; target?: any; artifactList: any[]; setArtifactList: (updater: any) => void; elements: any[]; setElements: (updater: any) => void; kitEdits: Record<string, any>; setKitEdits: (updater: any) => void; stages: any[]; setStages: (updater: any) => void; extraKits: any[]; setExtraKits: (updater: any) => void }) {
...
function BrandScreen({ displayKits, stages, elements, artifacts, period }: { displayKits: any[]; stages: any[]; elements: any[]; artifacts: any[]; period?: { from: string; to: string } }) {
  ...
  const donePeriod = allSteps.filter((step: any) => step.status === 'done' && inPeriod(step.completedAt ?? today, periodRange)).length;
  ...
  const trendRows = Array.from({ length: visibleTrendDays }, (_, index) => {
    ...
    const finished = allSteps.filter((step: any) => step.status === 'done' && (step.completedAt ?? today) === iso).length;
    const active = allSteps.filter((step: any) => step.status === 'in-progress' && stepDate(step) === iso).length;
    return { iso, day: date.getDate(), added, finished, active };
  });
  const trendMax = Math.max(1, ...trendRows.map(day => Math.max(day.added, day.finished, day.active)));
  const kitRows = displayKits.map((kit: any) => {
    ...
    const stats = kitStepStats(stageList, kit.id, periodRange);
    ...
    return { id: kit.id, name: kit.name, color: kit.color?.border ?? '#60a5fa', total: stats.total, done: stats.done, readiness: stats.readiness, active, waiting, blockers, addedPeriod: stats.addedPeriod, donePeriod: stats.donePeriod, periodNet: stats.donePeriod - stats.addedPeriod };
  ...
  const graphLinks = graphElements.flatMap((element: any) => (element.kits ?? []).filter((kitId: string) => visibleKitIds.has(kitId)).map((kitId: string) => ({ source: kitId, target: element.id, lineStyle: { color: kitColor(kitId, displayKits), opacity: 0.58, width: 1.4 } })));
  const graphOption = {
    backgroundColor: 'transparent',
    ...
    series: [{
      ...
      roam: false,
      data: [
        ...displayKits.map((kit: any) => ({ id: kit.id, name: kit.name, category: 0, symbol: 'circle', symbolSize: 58, label: { show: true, fontWeight: 800, fontSize: 11, color: '#f8fafc' }, itemStyle: { color: kit.color?.border ?? '#60a5fa' } })),
        ...graphElements.map((element: any) => {
          ...
          return { id: element.id, name: element.name, category: element.status === 'unique' ? 2 : element.status === 'locked' ? 3 : 1, symbol: colors.length > 1 ? pieSymbol(colors) : 'circle', symbolSize: element.status === 'unique' || element.status === 'locked' ? 30 : 24, label: { show: true, fontSize: 10, color: '#cbd5e1' }, itemStyle: { color: element.status === 'unique' ? '#ef4444' : element.status === 'locked' ? '#f97316' : colors[0] ?? '#64748b' } };
        })
      ],
      links: graphLinks,
  ...
  return <main className="screen fit-screen brand-screen">
    <section className="brand-exec-top">
      ...
      <Panel className="brand-status-card brand-volume-card">
        ...
        <div className="brand-volume-stack" aria-label={`Объем: готово ${done}, в работе ${inWork}, ожидает ${waiting}, блокер ${blocked}`}>
          {statusRows.map(row => row.value > 0 ? <span key={row.className} className={`brand-volume-segment ${row.className}`} style={{ flex: row.value }}><i>{row.value}</i></span> : null)}
          {!allSteps.length && <span className="brand-volume-segment empty"><i>0</i></span>}
        </div>
        <div className="brand-volume-bars">
      ...
      <Panel className="brand-status-card brand-flow-card">
        ...
        <div className={`brand-delta ${periodNet > 0 ? 'good' : periodNet < 0 ? 'danger' : 'flat'}`}>{periodNet > 0 ? `Остаток −${periodNet}` : periodNet < 0 ? `Остаток +${Math.abs(periodNet)}` : 'Остаток 0'}</div>
      </Panel>
      <Panel className="brand-status-card">
        <div className="brand-section-head"><span>Консолидация</span><b>{merge.percent}%</b></div>
        <div className="brand-merge-line"><span style={{ width: `${merge.percent}%` }}/></div>
        <div className="brand-muted-line">{merge.common} общих · {merge.unique} уник. · {artifacts.length} арт.</div>
      </Panel>
    </section>
    <section className="brand-exec-grid">
      ...
      <div className="brand-side-stack">
        <Panel className="brand-dynamics-panel">
          ...
          <div className="brand-flow-strip" style={{ gridTemplateColumns: `repeat(${visibleTrendDays}, minmax(12px, 1fr))` }}>
            {trendRows.map(day => <div key={day.iso} className={`brand-flow-day ${day.iso === today ? 'today' : ''}`} title={`${day.iso}: +${day.added}, ✓${day.finished}, в работе ${day.active}`}>
              <span className="brand-flow-bars"><i className="add" style={{ height: day.added ? `${Math.max(6, day.added / trendMax * 100)}%` : 0 }}/><i className="done" style={{ height: day.finished ? `${Math.max(6, day.finished / trendMax * 100)}%` : 0 }}/><i className="work" style={{ height: day.active ? `${Math.max(6, day.active / trendMax * 100)}%` : 0 }}/></span>
              <b>{day.day}</b>
            </div>)}
          </div>
          <div className="brand-calendar-legend"><span><i className="add"/> добавлено</span><span><i className="done"/> закрыто</span><span><i className="work"/> в работе</span></div>
        </Panel>
